...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        return [VariableWithDeriv(inp, d_input) for inp,
                d_input in zip(inputs, d_output) if not is_constant(inp)]
...
def backpropagate(final_variable_with_deriv):
    ...
    while queue:
        var_derivate = queue.pop(0)
        var, derivate = var_derivate.variable, var_derivate.deriv
        if is_leaf(var):
            var._add_deriv(derivate)
        else:
            queue.extend(var.history.backprop_step(derivate))
