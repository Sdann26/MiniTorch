...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    """
    forward = list([vals[i] if i != arg else vals[i] + epsilon for i in range(0, len(vals))])
    backward = list([vals[i] if i != arg else vals[i] - epsilon for i in range(0, len(vals))])
...
def derivative_check(f, *scalars):
    ...
    for i, x in enumerate(scalars):
        ...
        np.testing.assert_allclose(x.derivative, check.data, 1e-1, 1e-1)
